...
const sb = {
  ...
  delete: (table, params) => sb.query(`${table}?${params}`, { method: "DELETE" }),
...
/* ── Header ── */
.hdr{background:linear-gradient(180deg,#0c1322 0%,#080d18 100%);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:200;backdrop-filter:blur(12px)}
...
.admin-score{display:flex;align-items:center;gap:6px}
.admin-score label{font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px}
...
.ac-pred{background:#0f2d1a;color:#4ade80}
...
.score-override-row{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center;padding:8px 14px;border-bottom:1px solid #ffffff06;font-size:12px}
.score-override-row:last-child{border-bottom:none}
...
.auth-card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:40px 36px;width:100%;max-width:400px}
...
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#0f2d1a;color:#4ade80;border:1px solid #166534;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:600;z-index:999;animation:fadeup .3s ease}
  ...
  async function submit() {
    ...
    try {
      if (mode === "signup") {
        ...
        await sb.post("users", { emp_id: id, name: name.trim() });
        await sb.post("sessions", {
          emp_id: id, device_id: deviceId,
    ...
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <div className="auth-toggle">
          {mode === "login"
            ? <>First time? <span onClick={() => { setMode("signup"); setErr(""); }}>Sign up</span></>
...
function MatchCard({ match, user, myPred, isAdmin, onScoreOverride, onToast }) {
  ...
  async function savePred() {
    ...
    try {
      ...
      setSaved(true);
      onToast("✓ Prediction saved!");
    } catch (e) { onToast("❌ Failed to save"); }
    setSaving(false);
  }
  ...
  return (
    <div className={`mcard${isLive ? " live" : isDone ? " done" : ""}`}>
      ...
      <div className="mcard-foot">
        ...
        {pts !== null && (
          <span className={`pts-chip pts-${pts}`}>
            {pts === 2 ? "⭐" : pts === 1 ? "✓" : "✗"} {pts} pt{pts !== 1 ? "s" : ""}
          </span>
        )}
        ...
        {/* Admin score override */}
        {isAdmin && (
          <div className="admin-score" style={{marginLeft:"auto"}}>
            <label style={{fontSize:10,color:"var(--muted)"}}>ADMIN:</label>
...
function MyPicks({ user, matches, myPreds, onToast }) {
  ...
  return (
    <div>
      ...
      {predicted.length === 0
        ? <div className="empty">You haven't made any predictions yet — head to Matches!</div>
...
function AdminPanel({ matches, allUsers, allPreds, sessions, auditLog, onScoreOverride, onToast }) {
  ...
  const actionClass = { login:"ac-login", prediction:"ac-pred", signup:"ac-signup", score:"ac-score" };
    ...
    <div>
        ...
        <div className="admin-grid">
          <div className="panel">
            ...
            <div className="stat-grid">
              {[["Users",allUsers.length],["Predictions",totalPreds],["Active (24h)",activeLast24],["Completed",completed],["Duplicate Devices",duplicates.length],["Audit Events",auditLog.length]].map(([l,v]) => (
                <div key={l} className="scard"><div className="scard-n" style={l==="Duplicate Devices"&&v>0?{color:"var(--red)"}:{}}>{v}</div><div className="scard-l">{l}</div></div>
              ))}
            </div>
          </div>
          <div className="panel">
              ...
              {allUsers.slice(0,8).map((u,i) => {
                ...
                return (
                  <div key={u.emp_id} className="log-row">
                    ...
                    <div className="log-time" style={{color:"var(--gold)",fontWeight:700}}>{pts} pts</div>
                  </div>
                );
              })}
            </div>
      ...
      {view === "devices" && (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {duplicates.map(([devId, emps]) => (
                  <div key={devId} className="dev-row">
                    <div>
                      ...
                      <div className="dev-info">Same device: {devId.slice(0,20)}...</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="panel">
            <div className="panel-hd">📱 All Device Sessions</div>
            <div className="panel-body">
              ...
              {[...sessions].reverse().map((s,i) => (
                <div key={i} className="dev-row">
                  <div>
                    ...
                    <div className="dev-info">{parseUA(s.user_agent||"")} · {parseBrowser(s.user_agent||"")} · ID: {s.device_id?.slice(0,16)}...</div>
                  </div>
      ...
      {view === "log" && (
        <div className="panel">
          <div className="panel-hd">📋 Audit Log <span style={{fontSize:11,color:"var(--muted)",fontFamily:"Inter",fontWeight:400}}>{auditLog.length} events</span></div>
          <div className="panel-body">
            {auditLog.length === 0 && <div className="empty">No events yet</div>}
            {[...auditLog].reverse().map((l,i) => (
              <div key={i} className="log-row">
                <div className="log-emp">{l.emp_id || "—"}</div>
      ...
      {view === "scores" && (
        <div className="panel">
          <div className="panel-hd">⚽ Manual Score Override <span style={{fontSize:11,color:"var(--muted)",fontFamily:"Inter",fontWeight:400}}>— backup if API fails</span></div>
          <div className="panel-body">
            {matches.filter(m=>m.status!=="completed"&&m.home!=="TBD").slice(0,20).map(m => (
  ...
  return (
    <div style={{padding:"40px 0",textAlign:"center"}}>
      <div style={{display:"inline-flex",flexDirection:"column",gap:10,maxWidth:300,width:"100%"}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:"var(--gold)"}}>🔐 Admin Access</div>
        <input className="field" style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text)",padding:"10px 14px",fontSize:14,fontFamily:"Inter"}}
          type="password" placeholder="Admin password" value={pw} onChange={e=>{setPw(e.target.value);setErr(false)}}
...
export default function App() {
  ...
  async function handleScoreOverride(matchId, hs, as_) {
    await audit("ADMIN", "score", `Match ${matchId} set to ${hs}-${as_}`);
  ...
  const tabs = [["matches","Matches"],["my","My Picks"],["board","Leaderboard"],["admin","Admin 🔐"]];
        ...
        {tab === "matches" && (
          <div>
            ...
            <div className={`api-bar${apiOk?"":" err"}`}>
              ...
              {apiOk ? "Live data from worldcup26.ir · Auto-refreshes every 60s" : "⚠️ Live API unreachable — showing demo data · Real scores load automatically when API recovers"}
            </div>
            {loading
